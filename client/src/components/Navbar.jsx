...
const Navbar = () => {
  ...
  const {setShowLogin, token, user, logout, isOwner, axios, setIsOwner} = useAppContext ()
  ...
  return (
    ...
    text-gray-600 border-b border-borderColor relative transition-all ${location.pathname === '/' && "bg-light"}`}>
      <Link to='/'>
        ...
        <div className={`max-sm:fixed max-sm:w-full max-sm:h-screen max-sm:top-16 max-sm:border-t
        ...
        max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === '/' ? "bg-light" : "bg-white"}
         ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
          ...
          <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
            ...
            <button onClick={()=> {token ? logout() : setShowLogin(true)}} className='cursor-pointer px-8 py-2 bg-primary
             hover:bg-primary-dull text-white transition-all
             rounded-lg'>{token ? 'Logout' : 'Login'}</button>
          </div>
        </div>
